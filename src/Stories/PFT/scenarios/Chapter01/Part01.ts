...
scenarioManager.addScenario({ storyName: EStoriesEn.PFT, chapterName: 'Глава 1', partName: 'Часть 1', code: '0' }, [
...

  {
    ...
    text:
      `
        «Ну, всё-таки и сон! Сам дьявол — добряк, если сравнивать с садистом у изголовья кушетки. Ещё трясёт... В висках покалывает, будто на самом деле были электроды, что за бред…»
      `,
    buttons: [
      {
        text: '',
...

  {
    ...
    text:
      `
        «Теперь не усну. А жаль, так бы время шло быстрее».
      `,
    buttons: [
      {
        text: '',
...

  {
    ...
    condition: [
      ...
      },
      {
        goTo: 800,
        condition: () => {
          return statsManager.get({ story: EStoriesEn.PFT, category: 'Choice', id: 'LikeReading' }) <= 0 && statsManager.get({ story: EStoriesEn.PFT, category: 'Choice', id: 'GoodNight' }) >= 1
        }
      },
      {
        goTo: 801,
        condition: () => {
          return statsManager.get({ story: EStoriesEn.PFT, category: 'Choice', id: 'LikeReading' }) >= 1 && statsManager.get({ story: EStoriesEn.PFT, category: 'Choice', id: 'GoodNight' }) <= 0
        }
      }
    ],
    imageBack: require('../../../../Media/Images/PFT/Backgrounds/Chamber_Night.jpg')
...

  {
    ...
    text:
      `
        Пара неуверенных движений в густом непрозрачном воздухе и её кулак сжала чья-то липкая рука. А потом, дёрнув с нечеловеческой силой, заставила упасть на пол.
      `,
    buttons: [
      {
        text: '',
...

  {
    ...
    ambient: require('../../../../Media/Audio/PFT/Ambients/Fireplace.mp3'),
    imageBack: require('../../../../Media/Images/PFT/Backgrounds/Room_Fireplace_Night.jpg')
  },
...

  {
    ...
    beforeBegin: () => {
      timer.set(10, () => { scenarioManager.beginScene(284) })
    },
    music: require('../../../../Media/Audio/PFT/Music/Stress_Theme.mp3'),
...

  {
    ...
    text:
      `
        — Добрый день. Вы потеряли сознание в душевой, ударились при падении головой и не приходили в себя. Медсестра отправилась за дежурным врачом — за мной. Доктор Натан Герш, приятно познакомиться.
      `,
    buttons: [
      {
        text: '',
...

  {
    ...
    text:
      `
        «Бюджет явно ограничен. Лишь меня так кормят? Но повар на кухне определённо добрый человек».
      `,
    buttons: [
      {
        text: '',
...

  {
    ...
    speakerL: 'Венди',
    simple: require('../../../../Media/Audio/PFT/Sounds/Kabli.mp3'),
    fullscreenObjectL: require('../../../../Media/Images/PFT/Persons/Wendy_Hospital.png'),
    imageBack: require('../../../../Media/Images/PFT/Backgrounds/Chamber_Night.jpg')
  },
