...
export default class CTimer {
  ...
  constructor (private readonly soundManager: CSoundSystem, private readonly timerElement: HTMLElement, private readonly timerNumberElement: HTMLElement) {}

  set (time: number, failFunc: () => void): void {
    let leftTime = time
    this.soundManager.play('ambient', require('../Sounds/Common/Timer.mp3'))
    this.timerElement.setAttribute('style', `display: flex; --duration: ${time}s`)
