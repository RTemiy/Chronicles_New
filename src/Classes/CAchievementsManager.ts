...
export default class CAchievementsManager {
  ...
  isUnlockedSome (story: EStoriesEn, name: string[]): boolean {
    let result = true
    name.forEach(el => {
      !this.#achievements[story + '_' + el].unlocked && (result = false)
    })
    return result
  }
  ...
  getAchievementsHTML (story?: string): string {
    ...
    for (const prop in this.#achievements) {
      ...
      <div class="achievement story-${this.#achievements[prop].story} ${this.#achievements[prop].unlocked ? 'achievement_unlocked' : 'achievement_locked'}">
        <img class="achievement__image ${this.#achievements[prop].unlocked && 'shimmering_image'} " src="${this.#achievements[prop].unlocked ? this.#achievements[prop].image : getCurrentEventImageSVG('icon_lock')}">
